...
\begin{equations}
    mean(D) &= \frac{\sum D}{|D|} \\
    \Delta mean(D) &= \left| \frac{\sum D}{|D|} - \frac{\sum (D) - d}{|D| - 1} \right|
\end{equations}
...
% SAM COMMENT: I am not really happy with this section at all!! I will rewrite
% it later. A lot of these 'limitations' are simply untrue.
...
%         Future iterations of this project could implement more statistical
%         functions.
...
%     % SAM COMMENT: I am not sure if following limitation is really true. We have
%     % categorical data, and we can summarize it using `count`. We don't use
%     % exponential mech. for that as of now, but I am not sure if that is a
%     % problem. Plus, it would be pretty trivial to add if I cleaned things up.

%     % \item Our implementation is limited to the Laplace Mechanism. Adding
%     %    alternative differential privacy mechanism such as the Exponential
%     %    Mechanism might make what...?
%     \item Our implementation is not heavily optimized. The performance of our
%         database could be improved drastically in future versions.
%     % SAM COMMENT: What about this?
%     \item Out code base is not particularly extendable. Future implementations
%     could clean things up a lot.
% \end{itemize}

% % SAM COMMENT: What? It is not really *that* limited... we are not missing any
% % of the fundamental building-blocks. It is mostly a matter of adding more
% % functionality to the database. The DP portion of it is fully implemented.
